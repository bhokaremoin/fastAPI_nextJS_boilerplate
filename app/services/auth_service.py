...
class AuthService:
    ...
    def create_user(ip_data, request_payload: AuthRequest):
        hashed_password = bcrypt.hashpw(request_payload.password.encode(), bcrypt.gensalt())
        return UserService.create_user(session=db.session, email=request_payload.email.lower(),
